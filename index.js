...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++){
        ...
        gameCard.innerHTML = `<img class="game-img" src="${games[i].img}" /><h2>${games[i].name}</h2><p>${games[i].description}</p>`;
...
function filterFundedOnly() {
    ...
    let exceededGoal = GAMES_JSON.filter ((games) => {
        return games.pledged >= games.goal;
    });
...
const amountRaised = fundedGames.reduce((acc, games) => {
    return acc + games.backers;
}, 0);


// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of ${amountRaised.toLocaleString('en-US')} has been raised for ${fundedGames.length} games. Currently, ${underfundedGames.length} game(s) remain unfunded. We need your help to fund these amazing games!`;
...
const combinedGames = [...GAMES_JSON].sort((a, b) => b.pledged - a.pledged);
const [topGame, runnerUp] = sortedGames;

// create a new element to hold the name of the top pledge game, then append it to the correct element
let newElement = document.createElement('p');
newElement.innerHTML= `${topGame.name}`;
firstGameContainer.appendChild(newElement);

// do the same for the runner up item
let newElement2 = document.createElement('p');
newElement2.innerHTML= `${runnerUp.name}`;
